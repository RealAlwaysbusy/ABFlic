...
public class FlicClient {
     ...
     * @throws IOException
     */
    public void cancelScanWizard(ScanWizard scanWizard) throws IOException, SocketException {
        if (scanWizard == null) {
            throw new IllegalArgumentException("scanWizard is null");
        }
    ...
    public Bdaddr getBdaddr(String buttonID) {
    	for (Integer key : connectionChannels.keySet()) {
    		ButtonConnectionChannel channel = connectionChannels.get(key);
    		if (channel.getBdaddr().toString().equals(buttonID)) {
    			return channel.getBdaddr();
    		}
    	}
    	return null;
    }
